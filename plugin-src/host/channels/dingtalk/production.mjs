...
function harnessOrigin(webServer, configured) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('dsh-message-dingtalk requires an initialized DSH webServer port');
  }
...
function pluginPaths(config) {
  ...
  const root = resolve(config.dataDir ?? join(dshHome, 'integrations', 'dsh-message-dingtalk'));
  return {
    root,
...
export async function createProductionController(ctx, config = {}, internals = {}) {
  if (!ctx?.credentials) throw new TypeError('dsh-message-dingtalk requires ctx.credentials');
  if (!ctx?.webServer) throw new TypeError('dsh-message-dingtalk requires ctx.webServer');
  ...
  const logger = config.logger ?? (typeof ctx.logger === 'function'
    ? ctx.logger('dsh-message-dingtalk')
    : (ctx.logger ?? console));
  ...
  const coreController = new Controller({
    ...
    createRuntime: async ({ botId, config: botConfig, clientSecret }) => {
      ...
      const workspaceScope = createBotWorkspaceScope(harness, {
        botId, workspaces, state, agent: config.agent, channel: 'dingtalk',
      });
      return new Runtime({
        config: botConfig,
        clientSecret,
  ...
  const controller = createWorkspaceAwareController(coreController, { workspaces, stateFor, logger });
