  ...
  async #reconcile() {
    try {
      ...
      if (connected < configured) {
        ...
        this.#retryIndex += 1;
        this.#logger.warn?.(
          `[dsh-message-dingtalk] ${connected}/${configured} bots connected; retrying in ${delayMs}ms`,
        );
      ...
      this.#retryIndex += 1;
      this.#logger.warn?.(
        `[dsh-message-dingtalk] connection reconciliation failed; retrying in ${delayMs}ms`,
        error,
      );
      this.#schedule(delayMs);
    }
