  ...
  async #reconcile() {
    try {
      ...
      if (connected < configured) {
        ...
        this.#retryIndex += 1;
        this.#logger.warn?.(
          `[dsh-message-weixin] ${connected}/${configured} accounts connected; retrying in ${delayMs}ms`,
        );
      ...
      const delayMs = this.#retryDelays[Math.min(this.#retryIndex, this.#retryDelays.length - 1)];
      this.#retryIndex += 1;
      this.#logger.warn?.(`[dsh-message-weixin] connection reconciliation failed; retrying in ${delayMs}ms`, error);
      this.#schedule(delayMs);
    }
  }
}
