...
function harnessOrigin(webServer, configured) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('dsh-message-weixin requires an initialized DSH webServer port');
  }
...
function pluginPaths(config) {
  ...
  const root = resolve(config.dataDir ?? join(dshHome, 'integrations', 'dsh-message-weixin'));
  return {
    root,
...
export async function createProductionController(ctx, config = {}, internals = {}) {
  if (!ctx?.credentials) throw new TypeError('dsh-message-weixin requires ctx.credentials');
  if (!ctx?.webServer) throw new TypeError('dsh-message-weixin requires ctx.webServer');
  ...
  const coreController = new Controller({
    ...
    createRuntime: async ({ botId, config: accountConfig, token }) => {
      ...
      const workspaceScope = createBotWorkspaceScope(harness, {
        botId, workspaces, state, agent: config.agent, channel: 'weixin',
      });
      return new Runtime({
        api,
