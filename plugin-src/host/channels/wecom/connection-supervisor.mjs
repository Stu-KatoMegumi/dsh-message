  ...
  async #reconcile() {
    try {
      ...
      if (connected < configured) {
        ...
        this.#retryIndex += 1;
        this.#logger.warn?.(`[dsh-message:wecom] ${connected}/${configured} bots connected; retrying in ${delay}ms`);
        this.#schedule(delay);
        return;
      }
      this.#retryIndex = 0;
      ...
      const delay = this.#retryDelays[Math.min(this.#retryIndex, this.#retryDelays.length - 1)];
      this.#retryIndex += 1;
      this.#logger.warn?.(`[dsh-message:wecom] connection reconciliation failed; retrying in ${delay}ms`, error);
      this.#schedule(delay);
    }
  }
}
