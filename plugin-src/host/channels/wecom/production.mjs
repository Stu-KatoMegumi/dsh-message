...
function harnessOrigin(webServer, configured) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('dsh-message Enterprise WeChat requires an initialized DSH webServer port');
  }
...
export async function createProductionController(ctx, config = {}, internals = {}) {
  if (!ctx?.credentials) throw new TypeError('dsh-message Enterprise WeChat requires ctx.credentials');
  if (!ctx?.webServer) throw new TypeError('dsh-message Enterprise WeChat requires ctx.webServer');
  ...
  const logger = config.logger
    ?? (typeof ctx.logger === 'function' ? ctx.logger('dsh-message:wecom') : (ctx.logger ?? console));
  ...
  const coreController = new Controller({
    ...
    createRuntime: async ({ botId, config: botConfig, secret }) => {
      ...
      const workspaceScope = createBotWorkspaceScope(harness, {
        botId, workspaces, state, agent: config.agent, channel: 'wecom',
      });
      return new Runtime({
        config: botConfig,
        secret,
