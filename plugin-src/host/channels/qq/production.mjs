...
function harnessOrigin(webServer, configured) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('dsh-message QQ requires an initialized DSH webServer port');
  }
...
export async function createProductionController(ctx, config = {}, internals = {}) {
  if (!ctx?.credentials) throw new TypeError('dsh-message QQ requires ctx.credentials');
  if (!ctx?.webServer) throw new TypeError('dsh-message QQ requires ctx.webServer');
  ...
  const logger = config.logger
    ?? (typeof ctx.logger === 'function' ? ctx.logger('dsh-message:qq') : (ctx.logger ?? console));
  ...
  const coreController = new Controller({
    ...
    createRuntime: async ({ botId, config: botConfig, appSecret }) => {
      ...
      const workspaceScope = createBotWorkspaceScope(harness, {
        botId, workspaces, state, agent: config.agent, channel: 'qq',
      });
      return new Runtime({
        config: botConfig,
        appSecret,
