...
export class ConnectionSupervisor {
  ...
  async #reconcile() {
    ...
    try {
      ...
      if (current.connected < current.configured) {
        ...
        this.#retryIndex += 1;
        this.#logger.warn?.(
          `[dsh-message-feishu] ${current.connected}/${current.configured} bots connected; retrying automatically in ${delay}ms`,
        );
        this.#schedule(delay);
        return;
      }
  ...
  #retry(message, error) {
    ...
    this.#retryIndex += 1;
    this.#logger.warn?.(
      `[dsh-message-feishu] ${message}; retrying automatically in ${delay}ms`,
      error,
    );
    this.#schedule(delay);
  }
