  ...
  async #reconcile() {
    try {
      ...
      if (connected < configured) {
        ...
        this.#retryIndex += 1;
        this.#logger.warn?.(
          `[dsh-message:${this.#channel}] ${connected}/${configured} bots connected; retrying in ${delay}ms`,
        );
        this.#schedule(delay);
        return;
      }
      ...
      this.#retryIndex += 1;
      this.#logger.warn?.(
        `[dsh-message:${this.#channel}] connection reconciliation failed; retrying in ${delay}ms`,
        error,
      );
      this.#schedule(delay);
    }
