...
export class QqController {
    ...
    for (const config of this.#configStore.list()) {
      await this.#withBotTransition(config.botId, async () => {
        ...
        try {
          ...
          this.#logger.warn?.(`[dsh-message:qq] bot ${config.botId} failed to initialize:`, error);
        } finally {
          this.#touch();
        }
      });
  ...
  async bindCredentials({ appId, appSecret } = {}) {
    ...
    await this.#withBotTransition(identity.botId, async () => {
      ...
      try {
        ...
        this.#logger.warn?.(`[dsh-message:qq] bot ${identity.botId} credential connection failed:`, error);
      }
      this.#touch();
    });
    return this.status();
  ...
  async deleteBot(botId) {
    ...
    await this.#withBotTransition(botId, async () => {
      ...
      await this.#deleteState({ botId, config }).catch((error) => {
        this.#logger.warn?.(`[dsh-message:qq] bot ${botId} state cleanup failed:`, error);
      });
      this.#errors.delete(botId);
      this.#touch();
    });
  ...
  async #completeProvisioning(record, credentials) {
    try {
      ...
      if (record.controller.signal.aborted) {
        ...
        this.#logger.error?.('[dsh-message:qq] provisioning failed:', error);
      }
    } finally {
      record.dispose?.();
      if (this.#activeAttemptId === record.id) this.#activeAttemptId = null;
  ...
  async #activateBot(record, { appId, appSecret, ownerUserOpenid }) {
    ...
    return this.#withBotTransition(identity.botId, async () => {
      ...
      try {
          ...
          if (previousConfig) await this.#configStore.save(previousConfig).catch(() => undefined);
            ...
            if (removed) {
              await this.#deleteState({ botId: identity.botId, config }).catch((cleanupError) => {
                this.#logger.warn?.('[dsh-message:qq] cancelled bot state cleanup failed:', cleanupError);
              });
            }
          }
          await this.#restoreCredential(identity.secretRef, previousSecret);
        ...
        this.#errors.set(identity.botId, safeError('connection-failed', 'QQ 机器人已绑定，消息连接暂未就绪。'));
        this.#logger.warn?.(`[dsh-message:qq] bot ${identity.botId} activation connection failed:`, error);
      }
      this.#touch();
  ...
  async #stopRuntime(botId) {
    ...
    await runtime?.stop().catch((error) => {
      this.#logger.warn?.(`[dsh-message:qq] bot ${botId} failed to stop cleanly:`, error);
    });
  }
