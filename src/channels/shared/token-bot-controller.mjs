...
export class TokenBotController {
    ...
    for (const config of this.#configStore.list()) {
      await this.#withBotTransition(config.botId, async () => {
        ...
        try {
          ...
          this.#logger.warn?.(
            `[dsh-message:${this.#descriptor.key}] bot ${config.botId} failed to initialize:`,
            error,
          );
        } finally {
          this.#touch();
  ...
  async bindCredentials({ token } = {}) {
    ...
    await this.#withBotTransition(identity.botId, async () => {
      ...
      try {
        ...
        this.#logger.warn?.(
          `[dsh-message:${this.#descriptor.key}] bot ${identity.botId} credential connection failed:`,
          error,
        );
      }
      this.#touch();
  ...
  async deleteBot(botId) {
    ...
    await this.#withBotTransition(botId, async () => {
      ...
      await this.#deleteState({ botId, config }).catch((error) => {
        this.#logger.warn?.(
          `[dsh-message:${this.#descriptor.key}] bot ${botId} state cleanup failed:`,
          error,
        );
      });
      this.#errors.delete(botId);
  ...
  async #stopRuntime(botId) {
    ...
    await runtime?.stop().catch((error) => {
      this.#logger.warn?.(
        `[dsh-message:${this.#descriptor.key}] bot ${botId} failed to stop cleanly:`,
        error,
      );
    });
  }
