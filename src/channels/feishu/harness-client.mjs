...
export class HarnessClient {
  ...
  constructor({ baseUrl, workspace, agentPreset, autostart, dshBin, logger = console }) {
  ...
  async ensureRunning() {
    ...
    if (!this.#managedProcess || this.#managedProcess.exitCode !== null) {
      ...
      this.#managedProcess.on('error', (error) => {
        this.#logger.error?.('[bridge] failed to start Harness:', error.message);
      });
    }
  ...
  async ask(sessionId, text, options = {}) {
    ...
    while (Date.now() < deadline) {
      ...
      if (update && onUpdate) {
        try {
          ...
          this.#logger.warn?.('[bridge] ignored a progress update failure:', error.message);
        }
      }
    ...
    throw createTurnFailure({ timeoutMs, context: '飞书' });
  }
