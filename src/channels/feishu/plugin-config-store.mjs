...
function normalizeDocument(value) {
  ...
  if (value.version === 2 && Array.isArray(value.bots)) {
    ...
    if (bots.some((bot) => bot === null)) {
      throw new Error('dsh-message-feishu config contains an invalid bot entry');
    }
    const ids = new Set();
    const refs = new Set();
    ...
    for (const bot of bots) {
      if (ids.has(bot.id) || refs.has(bot.secretRef) || appIds.has(bot.appId)) {
        throw new Error('dsh-message-feishu config contains duplicate bot identities');
      }
      ids.add(bot.id);
...
export class PluginConfigStore {
  ...
  async load() {
    try {
      ...
      if (!normalized) throw new Error('dsh-message-feishu config is incomplete or invalid');
  ...
  async save(value) {
    ...
    if (!normalized) throw new Error('Refusing to persist incomplete dsh-message-feishu configuration');
  ...
  async saveBot(value) {
    ...
    if (!normalized) throw new Error('Refusing to persist incomplete dsh-message-feishu bot configuration');
