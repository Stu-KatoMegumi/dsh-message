      ...
      client.registerCallbackListener(this.#topic, (response) => {
        ...
        if (callbackMessageId) {
          try {
            ...
            this.#logger.warn?.('[dsh-message-dingtalk] unable to acknowledge an inbound callback');
          }
        }
        ...
        const task = Promise.resolve().then(async () => {
          ...
          try {
            ...
            this.#logger.warn?.('[dsh-message-dingtalk] ignored an invalid callback payload');
            return;
          }
          ...
          this.#logger.error?.('[dsh-message-dingtalk] callback processing failed');
  ...
  async stop({ preserveError = false } = {}) {
    ...
    if (client) {
      try {
        ...
        this.#logger.warn?.('[dsh-message-dingtalk] DingTalk Stream disconnect failed');
      }
    }
