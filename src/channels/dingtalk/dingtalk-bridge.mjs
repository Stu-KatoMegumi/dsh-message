...
import { chunkReplyBubbles, splitReplyBubbles } from '../shared/reply-bubbles.mjs';
...
export class DingtalkHarnessBridge {
  ...
  async #process(message, messageId, sender, key) {
    ...
    try {
      ...
      const streamed = cardStarted && bubbles.length > 0 && await cardStream.finish(bubbles[0]);
      if (streamed) {
        for (const bubble of bubbles.slice(1)) await this.#send(sessionWebhook, bubble);
      } else {
      ...
      this.#logger.error?.('[dsh-message-dingtalk] failed to process an inbound message');
      try {
        ...
        this.#logger.error?.('[dsh-message-dingtalk] failed to send the safe error reply');
      }
    }
  }
  ...
  async #send(sessionWebhook, text) {
    const chunks = chunkReplyBubbles(
      text,
      (bubble) => splitDingtalkText(bubble, this.#maxMessageChars),
    );
