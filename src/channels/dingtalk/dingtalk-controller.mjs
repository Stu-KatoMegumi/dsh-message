...
export class DingtalkController {
    ...
    for (const config of this.#configStore.list()) {
      ...
      await this.#withBotTransition(config.botId, async () => {
        ...
        try {
          ...
          this.#logger.warn?.(`[dsh-message-dingtalk] bot ${latest.botId} failed to initialize`);
        }
        this.#touch();
      });
    }
  ...
  async startProvisioning({ signal } = {}) {
    if (this.#closed) throw new Error('dsh-message-dingtalk controller is closed');
  ...
  async bindCredentials({ clientId, clientSecret } = {}) {
    ...
    await this.#withBotTransition(identity.botId, async () => {
      ...
      try {
        ...
        this.#logger.warn?.('[dsh-message-dingtalk] credential-bound bot saved but its connection is not ready');
      }
      this.#touch();
    });
    return this.status();
  ...
  async deleteBot(botId) {
    ...
    await this.#withBotTransition(botId, async () => {
      ...
      try {
        ...
        this.#logger.warn?.(`[dsh-message-dingtalk] bot ${botId} state cleanup failed`);
      }
      this.#errors.delete(botId);
      this.#touch();
    });
  ...
  async #pollRegistration(record) {
    try {
      ...
      if (record.controller.signal.aborted || error?.name === 'AbortError') {
        ...
        if (this.#activeAttemptId === record.id) this.#activeAttemptId = null;
        this.#logger.error?.('[dsh-message-dingtalk] bot activation failed');
      } else {
        record.state = 'pending';
  ...
  async #activateBot(record, { clientId, clientSecret }) {
    ...
    return this.#withBotTransition(identity.botId, async () => {
        ...
        if (previousConfig) await this.#configStore.save(previousConfig).catch(() => undefined);
          ...
          if (removed) {
            await this.#deleteState({ botId: identity.botId, config }).catch((cleanupError) => {
              this.#logger.warn?.('[dsh-message-dingtalk] failed to clean up cancelled bot state:', cleanupError);
            });
          }
        }
        await this.#restoreCredential(identity.secretRef, previousSecret);
      ...
      try {
        ...
        this.#logger.warn?.('[dsh-message-dingtalk] authorized bot saved but its connection is not ready');
      }
  ...
  async #stopRuntime(botId) {
    ...
    await runtime?.stop().catch(() => {
      this.#logger.warn?.(`[dsh-message-dingtalk] bot ${botId} failed to stop cleanly`);
    });
  }
  ...
  async #restoreCredential(secretRef, previous) {
    try {
      ...
      this.#logger.error?.(`[dsh-message-dingtalk] failed to restore credential ${secretRef}`);
    }
  }
  ...
  #withBotTransition(botId, operation) {
    if (this.#closed) return Promise.reject(new Error('dsh-message-dingtalk controller is closed'));
