  ...
  async #start() {
    ...
    try {
      ...
      this.#monitor = this.#runMonitor(signal).catch((error) => {
        ...
        this.#logger.error?.(`[dsh-message-weixin] account ${this.#config.botId} monitor stopped:`, error);
      });
      return this.status;
    } catch (error) {
      this.#status.ready = false;
  ...
  async #runMonitor(signal) {
    ...
    while (!signal.aborted) {
      try {
        ...
        this.#status.lastError = error?.message ?? String(error);
        this.#logger.warn?.(
          `[dsh-message-weixin] account ${this.#config.botId} poll failed (${consecutiveFailures}/3):`,
          error,
        );
  ...
  async stop() {
    ...
    if (wasStarted) {
      try {
        ...
        this.#logger.warn?.(`[dsh-message-weixin] account ${this.#config.botId} stop notification failed:`, error);
      }
    }
    this.#status.ready = false;
