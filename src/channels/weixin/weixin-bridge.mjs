...
export class WeixinHarnessBridge {
  ...
  async #process(message) {
    ...
    try {
      ...
      this.#logger.error?.('[dsh-message-weixin] failed to process an inbound message:', error);
      try {
        await this.#send(sender, '消息处理失败，请稍后重试。', contextToken, runId);
        await this.#state.markSeen(messageId);
      } catch (sendError) {
        this.#logger.error?.('[dsh-message-weixin] failed to send the safe error reply:', sendError);
      }
    }
  }
  ...
  async #send(toUserId, text, contextToken, runId) {
    const chunks = chunkReplyBubbles(
      text,
      (bubble) => splitWeixinText(bubble, this.#maxMessageChars),
    );
