...
export class WeixinController {
    ...
    for (const config of this.#configStore.list()) {
      ...
      await this.#withBotTransition(config.botId, async () => {
        ...
        try {
          ...
          this.#logger.warn?.(`[dsh-message-weixin] account ${latest.botId} failed to initialize:`, error);
        } finally {
          this.#touch();
        }
      });
  ...
  async startProvisioning() {
    if (this.#closed) throw new Error('dsh-message-weixin controller is closed');
  ...
  async deleteBot(botId) {
    ...
    await this.#withBotTransition(botId, async () => {
      ...
      try {
        ...
        this.#logger.warn?.(`[dsh-message-weixin] account ${botId} state cleanup failed:`, error);
      }
      this.#errors.delete(botId);
      this.#touch();
    });
  ...
  async #runProvisioning(record) {
    try {
      ...
      if (record.controller.signal.aborted || error?.name === 'AbortError') {
        ...
        this.#logger.error?.('[dsh-message-weixin] provisioning failed:', error);
      }
    } finally {
      record.pendingVerifyCode = null;
      record.verifyResolve?.();
        ...
        if (previousConfig) await this.#configStore.save(previousConfig).catch(() => undefined);
          ...
          if (removed) {
            await this.#deleteState({ botId: identity.botId, config }).catch((cleanupError) => {
              this.#logger.warn?.('[dsh-message-weixin] failed to clean up cancelled bot state:', cleanupError);
            });
          }
        }
  ...
  async #stopRuntime(botId) {
    ...
    await runtime?.stop().catch((error) => {
      this.#logger.warn?.(`[dsh-message-weixin] account ${botId} failed to stop cleanly:`, error);
    });
  }
  ...
  async #restoreCredential(ref, previous) {
    try {
      ...
      this.#logger.error?.(`[dsh-message-weixin] failed to restore credential ${ref}:`, error);
    }
  }
