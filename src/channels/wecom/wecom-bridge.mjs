  ...
  async #process(frame) {
    ...
    try {
      ...
      try {
        ...
        this.#logger.warn?.('[dsh-message:wecom] unable to start a stream; using an active reply:', error);
      }
      ...
      if (streamStarted && chunks.length > 0) {
        try {
          ...
          this.#logger.warn?.('[dsh-message:wecom] stream finalization failed; using an active reply:', error);
        }
      }
      ...
      this.#logger.error?.('[dsh-message:wecom] failed to process an inbound message');
      try {
        ...
        this.#logger.error?.('[dsh-message:wecom] failed to send the safe error reply');
      }
    }
  }
}
