...
export class WecomRuntime {
  ...
  async #start(signal) {
    ...
    const onError = (error) => {
      ...
      this.#logger.warn?.(`[dsh-message:wecom] bot ${this.#config.botId} connection error`);
    };
  ...
  async #stopActive() {
    ...
    try {
      ...
      this.#logger.warn?.(`[dsh-message:wecom] bot ${this.#config.botId} failed to stop cleanly`);
    }
