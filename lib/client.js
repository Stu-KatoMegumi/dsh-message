...
function ModelSelect({ name: name2, label, description, value, options, busy, onChange }) {
  return h2(
    "label",
    ...
    h2(
      ...
      h2(
        "select",
        { name: name2, value, disabled: busy, onChange },
        options.map((option) => {
...
function AgentSettingsTab({ rpcCall }) {
  ...
  const updateModel = (name2, value) => {
    const choice = options.find((option) => modelValue(option) === value);
    if (choice) void invoke(ENDPOINTS.updateSettings, { settings: { [name2]: choice } });
  };
  return h2(
    "section",
